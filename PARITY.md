...
(★) items. Release automation separately verifies native FTR decoding, one
bundled ONNX Runtime/Silero inference, and bounded packaged application
startup. Every other capability requires the manual installed-app checks in
...
| Processing         | Normalize (loudnorm), leading-silence trim, fade in/out, high-pass filter, de-clip; released AI clarity chain: RNNoise denoise, bandwidth extension (FlashSR), and bounded per-channel auto-level analysis. DeepFilterNet is not distributed or selectable. DCCRN+ de-reverb is hidden/developer-provisioned and unavailable in the standard release. |
...
- GitHub Actions `release.yml` creates one commit-bound private draft, builds universal macOS and Windows installers sequentially, stages reviewed ONNX Runtime and FFmpeg inputs, smoke-tests a permission-cleared private FTR fixture, and uploads only through an exact-draft verifier. Tauri's action is build-only and has no release mutation credentials.
