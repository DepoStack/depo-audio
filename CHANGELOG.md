...
- **Bundled ONNX Runtime and the Rust binding now use the same 1.22 C API.** The app no longer requests an API newer than its bundled desktop runtime, macOS-specific CoreML code compiles in CI, and release builds run real Silero VAD inference against the staged native library before packaging.
