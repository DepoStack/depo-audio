...
- **Packaged native smoke tests now exercise both downloadable containers on each platform.** The workflow extracts the MSI, NSIS installer, DMG, and app archive, inspects the private FTR fixture with packaged ffprobe, exercises released encoders, rejects learned-model or ONNX Runtime payloads, checks macOS signatures and universal binaries, and removes fixture credentials before candidate processes start.
...
- **Draft evidence now inventories the contents extracted from the DMG, app archive, MSI, and NSIS installer.** The gate requires FFmpeg and JavaScript-notice evidence, rejects packaged TRM fixtures, every ONNX file, and known model bytes even when renamed, and proves the two macOS containers carry the same app payload.
